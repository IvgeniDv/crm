...
$id_num = $_POST['id_num'];
$active_debt = $_POST['active_debt'];
$debt_amount = $_POST['debt_amount'];
$active_surplus = $_POST['active_surplus'];
$surplus_amount = $_POST['surplus_amount'];
...
//check if client has a payment record set up
$sql_check = "SELECT *
...
if (mysqli_num_rows($result) < 1) {
    ...
    $sql = "INSERT INTO client_payment_balance
                        (`id_num`,
                        `client_name`,
                        `client_owes`,
                        `debt_amount`,
                        ...
                        `client_phone`)
            VALUES
            ('$id_num', '$first_name $last_name', '$active_debt', '$debt_amount ','$active_surplus', '$surplus_amount' , '$email', '$phone')";

}else{
    $sql = "UPDATE client_payment_balance
        SET
        `client_owes` = '$active_debt',
        `debt_amount` = '$debt_amount',
        `client_in_surplus` = '$active_surplus',
...
if($result){
    $message = "Client payment balance updated successfully";
